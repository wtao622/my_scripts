...
const scdcghd4Arr = [],scdcgbody4Arr = [],scdcgspbody4Arr = [],scdcgcount = ''
let scdcghd4 = $.getdata('scdcghd4')
let scdcgbody4 = $.getdata('scdcgbody4')
let scdcgspbody4 = $.getdata('scdcgspbody4')
const sphd = {
'Accept' : `*/*`,
...
!(async () => {
  if (typeof $request !== "undefined") {
    ...
    scdcghd4Arr.push($.getdata('scdcghd4'))
    scdcgbody4Arr.push($.getdata('scdcgbody4'))
    scdcgspbody4Arr.push($.getdata('scdcgspbody4'))
    let scdcgcount = ($.getval('scdcgcount') || '1');
  for (let i = 2; i <= scdcgcount; i++) {
    scdcghd4Arr.push($.getdata(`scdcghd4${i}`))
    scdcgbody4Arr.push($.getdata(`scdcgbody4${i}`))
    scdcgspbody4Arr.push($.getdata(`scdcgspbody4${i}`))
  }
    console.log(`------------- 共${scdcghd4Arr.length}个账号-------------\n`)
      for (let i = 0; i < scdcghd4Arr.length; i++) {
        if (scdcghd4Arr[i]) {
          scdcghd4 = scdcghd4Arr[i];
          scdcgbody4 = scdcgbody4Arr[i];
          scdcgspbody4 = scdcgspbody4Arr[i];
          $.index = i + 1;
          console.log(`\n开始【食材大冲关${$.index}】`)
...
function scdcgck() {
  ...
  const scdcghd4 = JSON.stringify($request.headers)
        if(scdcghd4)    $.setdata(scdcghd4,`scdcghd4${status}`)
$.log(scdcghd4)
const scdcgbody4 = $request.body
        if(scdcgbody4)    $.setdata(scdcgbody4,`scdcgbody4${status}`)
$.log(scdcgbody4)
   $.msg($.name,"",'食材大冲关'+`${status}` +'红包数据获取成功！')
  }else if ($request.url.indexOf("reward_video/reward/") > -1) {
 
const scdcgspbody4 = $request.body
        if(scdcgspbody4)    $.setdata(scdcgspbody4,`scdcgspbody4${status}`)
$.log(scdcgspbody4)
