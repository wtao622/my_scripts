...
const scdcghd2Arr = [],scdcgbody2Arr = [],scdcgspbody2Arr = [],scdcgcount = ''
let scdcghd2 = $.getdata('scdcghd2')
let scdcgbody2 = $.getdata('scdcgbody2')
let scdcgspbody2 = $.getdata('scdcgspbody2')
const sphd = {
'Accept' : `*/*`,
...
!(async () => {
  if (typeof $request !== "undefined") {
    ...
    scdcghd2Arr.push($.getdata('scdcghd2'))
    scdcgbody2Arr.push($.getdata('scdcgbody2'))
    scdcgspbody2Arr.push($.getdata('scdcgspbody2'))
    let scdcgcount = ($.getval('scdcgcount') || '1');
  for (let i = 2; i <= scdcgcount; i++) {
    scdcghd2Arr.push($.getdata(`scdcghd2${i}`))
    scdcgbody2Arr.push($.getdata(`scdcgbody2${i}`))
    scdcgspbody2Arr.push($.getdata(`scdcgspbody2${i}`))
  }
    console.log(`------------- 共${scdcghd2Arr.length}个账号-------------\n`)
      for (let i = 0; i < scdcghd2Arr.length; i++) {
        if (scdcghd2Arr[i]) {
          scdcghd2 = scdcghd2Arr[i];
          scdcgbody2 = scdcgbody2Arr[i];
          scdcgspbody2 = scdcgspbody2Arr[i];
          $.index = i + 1;
          console.log(`\n开始【食材大冲关${$.index}】`)
...
function scdcgck() {
  ...
  const scdcghd2 = JSON.stringify($request.headers)
        if(scdcghd2)    $.setdata(scdcghd2,`scdcghd2${status}`)
$.log(scdcghd2)
const scdcgbody2 = $request.body
        if(scdcgbody2)    $.setdata(scdcgbody2,`scdcgbody2${status}`)
$.log(scdcgbody2)
   $.msg($.name,"",'食材大冲关'+`${status}` +'红包数据获取成功！')
  }else if ($request.url.indexOf("reward_video/reward/") > -1) {
 
const scdcgspbody2 = $request.body
        if(scdcgspbody2)    $.setdata(scdcgspbody2,`scdcgspbody2${status}`)
$.log(scdcgspbody2)
