...
const scdcghd1Arr = [],scdcgbody1Arr = [],scdcgspbody1Arr = [],scdcgcount = ''
let scdcghd1 = $.getdata('scdcghd1')
let scdcgbody1 = $.getdata('scdcgbody1')
let scdcgspbody1 = $.getdata('scdcgspbody1')
const sphd = {
'Accept' : `*/*`,
...
!(async () => {
  if (typeof $request !== "undefined") {
    ...
    scdcghd1Arr.push($.getdata('scdcghd1'))
    scdcgbody1Arr.push($.getdata('scdcgbody1'))
    scdcgspbody1Arr.push($.getdata('scdcgspbody1'))
    let scdcgcount = ($.getval('scdcgcount') || '1');
  for (let i = 2; i <= scdcgcount; i++) {
    scdcghd1Arr.push($.getdata(`scdcghd1${i}`))
    scdcgbody1Arr.push($.getdata(`scdcgbody1${i}`))
    scdcgspbody1Arr.push($.getdata(`scdcgspbody1${i}`))
  }
    console.log(`------------- 共${scdcghd1Arr.length}个账号-------------\n`)
      for (let i = 0; i < scdcghd1Arr.length; i++) {
        if (scdcghd1Arr[i]) {
          scdcghd1 = scdcghd1Arr[i];
          scdcgbody1 = scdcgbody1Arr[i];
          scdcgspbody1 = scdcgspbody1Arr[i];
          $.index = i + 1;
          console.log(`\n开始【食材大冲关${$.index}】`)
...
function scdcgck() {
  ...
  const scdcghd1 = JSON.stringify($request.headers)
        if(scdcghd1)    $.setdata(scdcghd1,`scdcghd1${status}`)
$.log(scdcghd1)
const scdcgbody1 = $request.body
        if(scdcgbody1)    $.setdata(scdcgbody1,`scdcgbody1${status}`)
$.log(scdcgbody1)
   $.msg($.name,"",'食材大冲关'+`${status}` +'红包数据获取成功！')
  }else if ($request.url.indexOf("reward_video/reward/") > -1) {
 
const scdcgspbody1 = $request.body
        if(scdcgspbody1)    $.setdata(scdcgspbody1,`scdcgspbody1${status}`)
$.log(scdcgspbody1)
