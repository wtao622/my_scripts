...
const scdcghd3Arr = [],scdcgbody3Arr = [],scdcgspbody3Arr = [],scdcgcount = ''
let scdcghd3 = $.getdata('scdcghd3')
let scdcgbody3 = $.getdata('scdcgbody3')
let scdcgspbody3 = $.getdata('scdcgspbody3')
const sphd = {
'Accept' : `*/*`,
...
!(async () => {
  if (typeof $request !== "undefined") {
    ...
    scdcghd3Arr.push($.getdata('scdcghd3'))
    scdcgbody3Arr.push($.getdata('scdcgbody3'))
    scdcgspbody3Arr.push($.getdata('scdcgspbody3'))
    let scdcgcount = ($.getval('scdcgcount') || '1');
  for (let i = 2; i <= scdcgcount; i++) {
    scdcghd3Arr.push($.getdata(`scdcghd3${i}`))
    scdcgbody3Arr.push($.getdata(`scdcgbody3${i}`))
    scdcgspbody3Arr.push($.getdata(`scdcgspbody3${i}`))
  }
    console.log(`------------- 共${scdcghd3Arr.length}个账号-------------\n`)
      for (let i = 0; i < scdcghd3Arr.length; i++) {
        if (scdcghd3Arr[i]) {
          scdcghd3 = scdcghd3Arr[i];
          scdcgbody3 = scdcgbody3Arr[i];
          scdcgspbody3 = scdcgspbody3Arr[i];
          $.index = i + 1;
          console.log(`\n开始【食材大冲关${$.index}】`)
...
function scdcgck() {
  ...
  const scdcghd3 = JSON.stringify($request.headers)
        if(scdcghd3)    $.setdata(scdcghd3,`scdcghd3${status}`)
$.log(scdcghd3)
const scdcgbody3 = $request.body
        if(scdcgbody3)    $.setdata(scdcgbody3,`scdcgbody3${status}`)
$.log(scdcgbody3)
   $.msg($.name,"",'食材大冲关'+`${status}` +'红包数据获取成功！')
  }else if ($request.url.indexOf("reward_video/reward/") > -1) {
 
const scdcgspbody3 = $request.body
        if(scdcgspbody3)    $.setdata(scdcgspbody3,`scdcgspbody3${status}`)
$.log(scdcgspbody3)
